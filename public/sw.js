...
self.addEventListener('fetch', event => {
  ...
  if (event.request.url.indexOf(url) > -1) {
    //cache with network with specific urls
    event.respondWith(
    ...
    //if requestUrl is part of Static Array
    // than respond with caches only strategy
    event.respondWith(caches.match(event.request));
  } else {
    //cache with network fallback stratey for all other urls
    event.respondWith(
      caches
        .match(event.request)
        .then(response => {
